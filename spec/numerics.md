...
| `q16_top_k(logits: [i32; N], k: u32) -> ([RouteId; k], [i32; k])` | bounded-K heap; single sequential scan over `N` with pinned worst-evict policy | `sequential` (left-to-right ingest) | n/a | Bounded-K heap is the chosen strategy (see `src/top_k.mind` §"Why bounded-heap and not argpartition+sort"); reduction order is pinned by ascending index iteration over `N`, then by the heap-invariant restoration order which is itself a sequential pass. RouteId IS already `SHA-256(external_id)` (set at catalog load), so the tie-break key is the raw 32-byte RouteId compared lexicographically left-to-right — no re-hashing inside the loop. |
