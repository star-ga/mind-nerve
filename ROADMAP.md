...
| p95 ≤ 30 ms on 4-core CPU | Native `cdylib` emit so the PyTorch encode-cost (~270 ms today) can be replaced by a Q16.16 native kernel | **0.3.0** — `--lib` cdylib, AOT codegen, MIC profile-locked headers | **mindc-side IN PROGRESS**. `--emit-shared` cdylib flag (`c444c77`) + Phase 0/1/1.5 std-surface intrinsics + P0d `Instr::FnDef`→`func.func` (`aacebe1`) + RFC 0005 **P0e Step 1** struct heap-record write path (`2f98a4f`) + RFC 0005 **P0f Step 1** `FieldAccess` read path (`c706a3e`, per-fn `struct_env` for local-`Ident` receivers, 6 tests, bench gate +4.06% / +1.6% / +0.46% under 5%) all landed. Remaining mindc blocker for full `std.vec`: **P0f Step 2** — chained access (`a.b.c`), `FieldAccess` of a function return, and struct-typed parameters, via either a fold-through-`StructLit` IR pass or a type-checker annotation. Multi-LLM consensus on the Step 2 architecture is the next gate. |
