...
//
// Phase A1.5 — SIMD encode path for the native Q16.16 encoder.
//
// SCORE-PATH RETIREMENT (plan U2): the former SIMD score-path symbols
// (__mind_nerve_blas_dot_q16_i64, __mind_nerve_blas_matmul_score_q16_i64,
// their packed-i32 cache, and the __mind_nerve_blas_reset_cache test hook)
// have been deleted. mn_encoder_score now runs a pure-MIND path over the
// compiler-emitted multithreaded intrinsic __mind_blas_gemv_q16_mt (see
// mind/exports/c_abi.mind); no C shim is on the score path. This file now
// carries ONLY the encode-path BLAS (matmul / qkt / attnv) plus the shared
// AVX2 dispatch hooks bound by the perf / byte-identity harness.
//
// The MIND-side Q16.16 buffers store one Q16.16 value per i64 slot
// (sign-extended; stride 8 bytes). The kernels read the low 32 bits of
// each i64 slot as the Q16.16 value (matching the Python wrapper which
// sign-extends i32 Q16.16 to i64 via numpy astype) and write Q16.16
// results sign-extended into i64 out-buffer slots (stride 8). This
// preserves the existing handle scratch-buffer ABI so c_abi.mind needs no
// layout change.
//
// Numerical contract:
//   * Q16.16 multiply -> Q32.32 (i64 widening) -> single arithmetic shift
//     right by 16 per output element. Same reduction semantics as the
//     tail-recursive matmul_q16 in mind/kernels/matmul_q16.mind.
